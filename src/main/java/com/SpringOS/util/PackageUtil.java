...
public class PackageUtil implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private List<ShiroResources> getResourceList(String packageName){
        ...
        RequiresPermissions rr;
        ShiroResources ss;
        for (Class clas : classes) {
            Method[] md = clas.getMethods();
            for (Method method : md) {
                rr = method.getAnnotation(RequiresPermissions.class);
                if (rr != null) {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        List<ShiroResources> list = new ArrayList<ShiroResources>();
        if(event.getApplicationContext().getParent() == null) {
            ...
            for (ShiroResources shiroResources: srList) {
                ...
                for(String str:set){//默认只取三层
                    try {
                        temp = (ShiroResources)shiroResources.clone();
                    } catch (CloneNotSupportedException e) {
                        temp = new ShiroResources();
                        temp.setRealName(shiroResources.getRealName());
                        temp.setMethod(shiroResources.getMethod());
                    }
                    temp.setShiroAuth(str);

                    tempShiroResources = shiroResourcesService.clearRepetitionData(temp);
                ...
                shiroResourcesService.save(list);
            }
        }
    }
    ...
    private Set<String> getAnalysis(String shiroResources){
        ...
        if(shiroResources.indexOf(":") > 0){
            String[] main = {
                    shiroResources.substring(0, shiroResources.lastIndexOf(":")),
                    shiroResources.substring(shiroResources.lastIndexOf(":")+1)
            };

            String[] tag = main[1].split(",");
            for (String str : tag) {
                set.add(main[0] + ":" + str);
            }
        }else{
